...
public class naveAmarilla2 : MonoBehaviour
{
    private Animator animator;
    public GameManager gameManager;
    public FondosManager fondoManager;
    public GameObject naveTurbo2;
    ...
    void Update()
    {
        ...
        if (fondoManager.SeleccionFondo == 3)
        {
            if (gameManager.start == true && gameManager.gameOver == false)
            {
                if (naveNueva == true)
                {
                    for (int i = 0; i < 1; i++)
                    {
                        Naves.Add(Instantiate(naveTurbo2, new Vector2(12 + i, 7), Quaternion.identity));
                        numeroNaves += 1;
                        naveNueva = false;
